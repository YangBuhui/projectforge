...
public class PluginAdminServiceImpl implements PluginAdminService
{
  ...
  {
    String activateds = LocalSettings.get().get("pf.plugins.active");
    if (activateds == null) {
      activateds = GlobalConfiguration.getInstance().getStringValue(ConfigurationParam.PLUGIN_ACTIVATED);
    }
