...
public class LdapMasterLoginHandlerTest extends AbstractTestBase
{
  ...
  @Autowired
  LdapMasterLoginHandler loginHandler;
  ...
  public void realTest()
  {
    ...
    /*
     * logon(TEST_ADMIN_USER); loginHandler.ldapConfig = new LdapConfig().setUserBase("ou=users"); loginHandler.userDao
     ...
     * groupDao.internalUpdate(group); synchronizeLdapUsers(loginHandler); ldapGroup = ldapGroupDao.findById(groupId1);
     ...
     * groups) { groupDao.internalMarkAsDeleted(g); } synchronizeLdapUsers(loginHandler); assertEquals(
     * "LDAP groups must be empty (all groups are deleted in the PF data-base).", 0,
     * ldapGroupDao.findAll(ldapRealTestHelper.ldapConfig.getGroupBase()).size()); final Collection<PFUserDO> users =
     * userGroupCache.getAllUsers(); for (final PFUserDO user : users) { userDao.internalMarkAsDeleted(user); }
     * synchronizeLdapUsers(loginHandler); assertEquals(
     * "LDAP users must be empty (all user are deleted in the PF data-base).", 0,
     * ldapUserDao.findAll(ldapRealTestHelper.ldapConfig.getGroupBase()).size()); ldapUser =
