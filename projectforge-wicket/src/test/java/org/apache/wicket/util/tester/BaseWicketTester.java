...
/**
 * A helper class to ease unit testing of Wicket applications without the need for a servlet container. See javadoc of
 * <code>WicketTester</code> for example usage. This class can be used as is, but JUnit users should use derived class
...
public class BaseWicketTester
{
  ...
  /**
   * Creates <code>WicketTester</code> and automatically create a <code>WebApplication</code>, but the tester will have
   * no home page.
   */
   ...
   * 
   * @param application a <code>WicketTester</code> <code>WebApplication</code> object
   */
   ...
   * 
   * @param application a <code>WicketTester</code> <code>WebApplication</code> object
   * @param servletContextBasePath the absolute path on disk to the web application's contents (e.g. war root) - may be
   *          <code>null</code>
   */
   ...
   * 
   * @param application a <code>WicketTester</code> <code>WebApplication</code> object
   * @param servletCtx the servlet context used as backend
   */
  ...
  {
    request = new Servlet3MockHttpServletRequest(application, httpSession, servletContext);
  ...
  /**
   * Cleans up feedback messages. This usually happens on detach, but is disabled in unit testing so feedback mesasges
   * can be examined.
   */
   ...
   * 
   * @param filter filter used to cleanup messages, accepted messages will be removed
   */
   ...
   * 
   * @param request request to process
   * @param forcedRequestHandler optional parameter to override parsing the request URL and force
   ...
   * <p>
   * You may subclass processRequest it, to monitor or change any pre-configured value. Request headers can be
   * configured more easily by calling {@link #addRequestHeader(String, String)}.
   * 
   * @param forcedRequest Can be null.
   * @param forcedRequestHandler Can be null.
   * @param redirect
   * @return true, if process was executed successfully
   */
  ...
  /**
   * Determine whether a given response contains a redirect leading to an external site (which cannot be replicated in
   * WicketTester). This is done by comparing the previous request's hostname with the hostname given in the redirect.
   * 
   * @param requestUrl request...
   * @param newUrl ...and the redirect generated in its response
  ...
  /**
   * Allows to set Request header value any time. They'll be applied (add/modify) on process execution
   * {@link #processRequest(MockHttpServletRequest, IRequestHandler, boolean)}. They are reset immediately after and
   * thus are not re-used for a sequence of requests.
   * <p>
   * Deletion (not replace) of pre-configured header value can be achieved by subclassing
   * {@link #processRequest(MockHttpServletRequest, IRequestHandler, boolean)} and modifying the request header
   * directly.
   * 
   * @param key
   * @param value
  ...
  /**
   * Renders the page specified by given {@link IPageProvider}. After render the page instance can be retrieved using
   * {@link #getLastRenderedPage()} and the rendered document will be available in {@link #getLastResponse()}.
   * 
   * Depending on {@link RenderStrategy} invoking this method can mean that a redirect will happen before the actual
   * render.
   * 
   * @param pageProvider
   * @return last rendered page
   ...
   * 
   * @param reference the resource reference to test
   ...
   * 
   * @param reference the resource reference to test
   * @param pageParameters the parameters passed to the resource reference
   * @return the tested resource reference
   */
  public ResourceReference startResourceReference(final ResourceReference reference,
   ...
   * <p>
   * In case the processed component was not a {@link Page} then the automatically created page markup gets removed. If
   * you need the whole returned markup in this case use {@link #getLastResponse()}
   * {@link MockHttpServletResponse#getDocument() .getDocument()}
   * </p>
   * 
  ...
  /**
   * This method tries to parse the last response to return the encoded base URL and will throw an exception if there
   * none was encoded.
   * 
  ...
  /**
   * @return <code>true</code> if redirect responses will be followed automatically, <code>false</code> otherwise.
   */
  ...
  /**
   * Encodes the {@link IRequestHandler} to {@link Url}. It should be safe to call this method outside request thread as
   * log as no registered {@link IRequestMapper} requires a {@link RequestCycle}.
   * 
   * @param handler
  ...
  /**
   * Simulates processing URL that invokes specified {@link RequestListenerInterface} on component.
   * 
   * After the listener interface is invoked the page containing the component will be rendered (with an optional
   * redirect - depending on {@link RenderStrategy}).
   * 
   * @param component
   * @param listener
   */
  ...
  /**
   * Simulates invoking a listener on a component. As opposed to the {@link #executeListener(Component)} method, current
  ...
  /**
   * Builds and processes a request suitable for invoking a listener. The <code>Component</code> must implement any of
   * the known <code>IListener</code> interfaces.
   * 
   ...
   * 
   * @param behavior an <code>AbstractAjaxBehavior</code> to execute
   */
   ...
   * 
   * @param <C>
   * @param pageClass a test <code>Page</code> class with default constructor
   ...
   * @param <C>
   * @param pageClass a test <code>Page</code> class with default constructor
   * @param parameters the parameters to use for the class.
   * @return the rendered <code>Page</code>
   */
  @SuppressWarnings("unchecked")
  public final <C extends Page> C startPage(final Class<C> pageClass,
   ...
   * 
   * @param path path to <code>FormComponent</code>
   ...
   * 
   * @param path path to <code>FormComponent</code>
   * @param fillBlankString specifies whether to fill all child <code>FormComponent</code>s with blank
   *          <code>String</code>s
   * @return a <code>FormTester</code> instance for testing the <code>Form</code>
   * @see FormTester
   */
   ...
   * <p>
   * <strong>Note</strong>: the instantiated component will have an auto-generated id. To reach any of its children use
   * their relative path to the component itself. For example if the started component has a child a Link component with
   * id "link" then after starting the component you can click it with: <code>tester.clickLink("link")</code>
   * </p>
   * 
   * @param <C> the type of the component
   * @param componentClass the class of the component to be tested
  ...
  /**
   * Process a component. A web page will be automatically created with the {@code pageMarkup} provided. In case
   * pageMarkup is null, the markup will be automatically created with {@link #createPageMarkup(String)}.
   * <p>
   * <strong>Note</strong>: the instantiated component will have an auto-generated id. To reach any of its children use
   * their relative path to the component itself. For example if the started component has a child a Link component with
   * id "link" then after starting the component you can click it with: <code>tester.clickLink("link")</code>
   ...
   * 
   * @param componentClass the class of the component to be tested
   * @param pageMarkup the markup for the Page that will be automatically created. May be {@code null}.
   * @return The component processed
   */
  public final <C extends Component> C startComponentInPage(final Class<C> componentClass,
   ...
   * {@link #createPageMarkup(String)}.
   * <p>
   * <strong>Note</strong>: the component id is set by the user. To reach any of its children use this id + their
   * relative path to the component itself. For example if the started component has id <em>compId</em> and a Link child
   * component component with id "link" then after starting the component you can click it with:
   * <code>tester.clickLink("compId:link")</code>
   * </p>
   * 
   * @param <C> the type of the component
  ...
  /**
   * Process a component. A web page will be automatically created with the {@code pageMarkup} provided. In case
   * {@code pageMarkup} is null, the markup will be automatically created with {@link #createPageMarkup(String)}.
   * <p>
   * <strong>Note</strong>: the component id is set by the user. To reach any of its children use this id + their
   * relative path to the component itself. For example if the started component has id <em>compId</em> and a Link child
   * component component with id "link" then after starting the component you can click it with:
  ...
  /**
   * Creates the markup that will be used for the automatically created {@link Page} that will be used to test a
   * component with {@link #startComponentInPage(Class, IMarkupFragment)}
   * 
   * @param componentId the id of the component to be tested
  ...
  /**
   * Creates a {@link Page} to test a component with {@link #startComponentInPage(Component, IMarkupFragment)}
   * 
  ...
  /**
   * A page that is used as the automatically created page for {@link BaseWicketTester#startComponentInPage(Class)} and
   * the other variations.
   * <p>
   * This page caches the generated markup so that it is available even after {@link Component#detach()} where the
   * {@link Component#markup component's markup cache} is cleared.
   */
   ...
   * 
   * Components which are somehow dependent on the page structure can not be currently tested with this method.
   * 
   * Example:
   * 
   * UserDataView view = new UserDataView("view", new ListDataProvider(userList)); tester.startComponent(view);
  ...
  /**
   * Gets the component with the given path from last rendered page. This method fails in case the component couldn't be
   * found.
   * 
   * @param path Path to component
   * @param wantVisibleInHierarchy if true component needs to be VisibleInHierarchy else null is returned
  ...
  /**
   * Gets the component with the given path from last rendered page. This method fails in case the component couldn't be
   * found, and it will return null if the component was found, but is not visible.
   * 
   * @param path Path to component
   * @return The component at the path
   * @see org.apache.wicket.MarkupContainer#get(String)
   ...
   * 
   * @param path path to <code>Label</code> component
   * @param expectedLabelText expected label text
   ...
   * 
   * @param pattern reqex pattern to match
   * @return a <code>Result</code>
   */
   ...
   * @param path Click the <code>Link</code> in the last rendered Page.
   */
   ...
   * <p>
   * On AjaxLinks and AjaxFallbackLinks the onClick method is invoked with a valid AjaxRequestTarget. In that way you
   * can test the flow of your application when using AJAX.
   * <p>
   * When clicking an AjaxSubmitLink the form, which the AjaxSubmitLink is attached to is first submitted, and then the
   * onSubmit method on AjaxSubmitLink is invoked. If you have changed some values in the form during your test, these
   * will also be submitted. This should not be used as a replacement for the {@link FormTester} to test your forms. It
   * should be used to test that the code in your onSubmit method in AjaxSubmitLink actually works.
   * <p>
   * This method is also able to simulate that AJAX (javascript) is disabled on the client. This is done by setting the
   * isAjax parameter to false. If you have an AjaxFallbackLink you can then check that it doesn't fail when invoked as
   * a normal link.
   * 
   * @param path path to <code>Link</code> component
   * @param isAjax Whether to simulate that AJAX (javascript) is enabled or not. If it's false then AjaxLink and
   *          AjaxSubmitLink will fail, since it wouldn't work in real life. AjaxFallbackLink will be invoked with null
   *          as the AjaxRequestTarget parameter.
   */
  public void clickLink(String path, boolean isAjax)
  {
    ...
    // if the link is a normal link (or ResourceLink)
      ...
      /*
       * If the link is a bookmarkable link, then we need to transfer the parameters to the next request.
       */
  ...
  /**
   * make url suitable for wicket tester use. usually this involves stripping any leading .. segments to make the url
   * absolute
   * 
   * @param url
   * @return Url
   ...
   * 
   * @param <C>
   * @param expectedRenderedPageClass expected class of last rendered page
   * @return a <code>Result</code>
   */
   ...
   * <p>
   * Use <code>-Dwicket.replace.expected.results=true</code> to automatically replace the expected output file.
   * </p>
   * 
   * @param pageClass used to load the <code>File</code> (relative to <code>clazz</code> package)
   * @param filename expected output <code>File</code> name
   * @throws Exception
   */
  ...
  /**
   * Asserts last rendered <code>Page</code> against an expected HTML document as a <code>String</code>.
   * 
   * @param expectedDocument expected output
   * @return a <code>Result</code>
   * @throws Exception
   */
   ...
   * 
   * @param level level of feedback message, for example:
  ...
  {
    List<FeedbackMessage> allMessages = new FeedbackCollector(getLastRenderedPage())
  ...
  /**
   * Dumps the <code>Component</code> trees to log. Show only the <code>Component</code>s whose paths contain the filter
   * <code>String</code>.
   * 
   * @param filter a filter <code>String</code>
   */
  public void debugComponentTrees(String filter)
  {
    ...
    for (WicketTesterHelper.ComponentData obj : WicketTesterHelper.getComponentData(getLastRenderedPage())) {
      if (obj.path.matches(".*" + filter + ".*")) {
        log.info("[{}{}] path\t" + obj.path + " \t" + obj.type + " \t[" + obj.value + "]", obj.isEnabled ? "E" : "-",
            obj.isVisible ? "V" : "-");
      }
    }
  }
   ...
   * {@link org.apache.wicket.ajax.AjaxRequestTarget#add(org.apache.wicket.Component...)}. This method actually tests
   * that a <code>Component</code> is on the Ajax response sent back to the client.
   * <p>
   * PLEASE NOTE! This method doesn't actually insert the <code>Component</code> in the client DOM tree, using
   * JavaScript. But it shouldn't be needed because you have to trust that the Wicket Ajax JavaScript just works.
   * 
   * @param component the <code>Component</code> to test
   * @return a <code>Result</code>
   */
   ...
   * 
   * @since 1.2.3
   * @param componentPath the <code>Component</code> path
   * @param event the event which we simulate being fired. If <code>event</code> is <code>null</code>, the test will
   *          fail.
   */
   ...
   * 
   * @param page the page which timers will be executed
   */
  ...
  /**
   * Simulates the firing of an Ajax event. You add an Ajax event to a <code>Component</code> by using:
   * 
   * <pre>
   *     ...
   ...
   * </pre>
   * 
   * You can then test that the code inside <code>onEvent</code> actually does what it's supposed to, using the
   * <code>WicketTester</code>:
   * 
   * <pre>
   *     ...
   ...
   * </pre>
   * 
   * This also works with <code>AjaxFormSubmitBehavior</code>, where it will "submit" the <code>Form</code> before
   * executing the command.
   * <p>
   * PLEASE NOTE! This method doesn't actually insert the <code>Component</code> in the client DOM tree, using
   * JavaScript.
   * 
   * @param component the <code>Component</code> that has the <code>AjaxEventBehavior</code> we want to test. If the
   *          <code>Component</code> is <code>null</code>, the test will fail.
   * @param event the event to simulate being fired. If <code>event</code> is <code>null</code>, the test will fail.
   */
  ...
  /**
   * Retrieves a <code>TagTester</code> based on a <code>wicket:id</code>. If more <code>Component</code>s exist with
   * the same <code>wicket:id</code> in the markup, only the first one is returned.
   * 
   * @param wicketId the <code>wicket:id</code> to search for
  ...
  /**
   * Modified version of BaseWicketTester#getTagByWicketId(String) that returns all matching tags instead of just the
   * first.
   * 
   * @param wicketId
   * @return List of Tags
  ...
  /**
   * Retrieves a <code>TagTester</code> based on an DOM id. If more <code>Component</code>s exist with the same id in
   * the markup, only the first one is returned.
   * 
   * @param id the DOM id to search for.
  ...
  /**
   * Helper method for all the places where an Ajax call should submit an associated <code>Form</code>.
   * 
   * @param component The component the behavior is attached to
   * @param behavior The <code>AjaxFormSubmitBehavior</code> with the <code>Form</code> to "submit"
   */
  ...
  /**
   * Puts all not already scheduled (e.g. via {@link FormTester#setValue(String, String)}) form component values in the
   * post parameters for the next form submit
   * 
   * @param form the {@link Form} which components should be submitted
   */
  ...
  /**
   * Rebuilds {@link ServletWebRequest} used by wicket from the mock request used to build requests. Sometimes this
   * method is useful when changes need to be checked without processing a request.
   */
  ...
  /**
   * Starts a page, a shared resource or a {@link IRequestListener} depending on what the {@link IRequestMapper}s
   * resolve for the passed url.
   * 
   * @param _url the url to resolve and execute
   */
